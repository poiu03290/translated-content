...
[links](/en-US/docs/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks)의 스타일을 지정할 때 가상 클래스를 사용하여 상태의 스타일을 효과적으로 지정하는 방법을 이해하는 것이 중요합니다. 탐색 메뉴 및 탭과 같이 콘텐츠가 다양한 공통 인터페이스 기능에서 사용할 링크의 스타일을 지정하는 방법을 아는 것도 중요합니다. 이 문서에서는 이 두 가지 주제를 모두 살펴보겠습니다.
...
<table>
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML"
          >HTML 소개</a
        > 둘러보기), CSS 기본 (
        <a href="/en-US/docs/Learn/CSS/First_steps">CSS 소개</a> 둘러보기),
    ...
    <tr>
      <th scope="row">목표:</th>
      <td>
        링크 상태의 스타일을 지정하는 방법과 탐색 메뉴와 같은 일반적인 UI 기능에서 링크를 효과적으로 사용하는 방법을 알아봅니다.
      </td>
    </tr>
  </tbody>
</table>
...
[Creating hyperlinks](/en-US/docs/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks)의 모범 사례에 따라 HTML에서 링크가 구현되는 방식을 살펴보았습니다. 이 문서에서는 이러한 지식을 바탕으로 스타일을 지정하는 모범 사례를 보여줍니다.

### 링크 상태

가장 먼저 이해해야 할 것은 링크 상태(링크가 존재할 수 있는 여러 상태)의 개념입니다. 서로 다른 [pseudo-classes](/en-US/docs/Learn/CSS/Building_blocks/Selectors#pseudo-classes)를 사용하여 스타일을 지정할 수 있습니다.

- **Link**: {{cssxref(":link")}} 의사 클래스를 사용하여 스타일이 지정된 대상(즉, 명명된 앵커가 아님)이 있는 링크입니다.
- **Visited**: 이미 방문한 적이 있는 링크(브라우저 기록에 있음) {{cssxref(":visited")}} 가상 클래스를 사용하여 스타일이 지정된 링크입니다.
- **Hover**: {{cssxref(":hover")}} 의사 클래스를 사용하여 스타일이 지정되고 사용자의 마우스 포인터로 가리킨 링크입니다.
- **Focus**: 포커스가 있는 링크(예: <kbd>Tab</kbd> 키 또는 이와 유사한 것을 사용하여 키보드 사용자가 이동하거나 {{domxref("HTMLElement.focus()")}}를 사용하여 프로그래밍 방식으로 포커스가 설정됨) — 이것은 {{cssxref(":focus")}} 가상 클래스를 사용하여 스타일이 지정됩니다.
- **Active**: {{cssxref(":active")}} 의사 클래스를 사용하여 스타일이 지정된 활성화된(예: 클릭) 링크입니다.
